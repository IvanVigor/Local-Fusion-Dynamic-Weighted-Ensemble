...
X_train, y_train, X_neighbours, y_neighbours, X_test, y_test = split_sets(
    california_housing.frame.sample(frac=1),
    0.70,
    0.20,
    0.10,
...
model_one = make_pipeline(StandardScaler(), RidgeCV(alphas=alphas))

model_one.fit(X_train, y_train)
TreeRegressor_one = DecisionTreeRegressor(max_depth=3, random_state=123)
TreeRegressor_two = DecisionTreeRegressor(max_depth=5, random_state=123)
...
X_test["one_preds_rf"] = TreeRegressor_one.predict(
    X_test[california_housing.feature_names]
)
X_test["two_preds_rf"] = TreeRegressor_two.predict(
    X_test[california_housing.feature_names]
)
X_test["one_preds_r"] = model_one.predict(X_test[california_housing.feature_names])

X_neighbours["one_preds_rf"] = TreeRegressor_one.predict(
    X_neighbours[california_housing.feature_names]
)
X_neighbours["two_preds_rf"] = TreeRegressor_two.predict(
    X_neighbours[california_housing.feature_names]
)
X_neighbours["one_preds_r"] = model_one.predict(
    X_neighbours[california_housing.feature_names]
)
...
ensemble.fit(
    X_neighbours,
    y_neighbours,
    features=california_housing.frame.drop(
        columns=[california_housing.target_names[0]]
    ).columns,
)
